...
import {
  templateStatusColorMap,
  templateStatusLabelMap,
...
const Templates = () => {
  ...
  const { data: statusWiseCounts, reload: reloadStatusCount } = useAPI<{
    ...
    statusCode: number;
    message: string;
  }>("b2b/v1/plembox/external/templates/count");
  const navigate = useNavigate();

  const { state, gotoPage, reload, search } = usePaginatedData<any>({
    endpoint: `b2b/v1/plembox/external/templates`,
    getUrl: (o) => {
      return `b2b/v1/plembox/external/templates?page=${o.page}&size=${o.limit}${
  ...
  const renderMobileCard = (item: any) => (
    <Badge.Ribbon
      text={templateStatusLabelMap[item.status]}
      color={templateStatusColorMap[item.status]}
      key={item.templateId}
    >
      <Card
        className="m-1"
        ...
        <Col span={24} className="mt-4">
          ...
          >
            <TemplateTableTabs
              ...
              statusCounts={statusWiseCounts?.data || {}}
            />
